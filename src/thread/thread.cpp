...
namespace fc {
    ...
    thread::thread(const std::string &name) {
        ...
        boost::thread *t = new boost::thread([this, p, name]() {
            try {
                ...
                wlog("unhandled exception");
                p->set_exception(std::make_shared<unhandled_exception>(
                        FC_LOG_MESSAGE(warn, "unhandled exception: ${diagnostic}",
    ...
    void thread::quit() {
        ...
        while (my->blocked) {
            ...
            while (cur) {
                ...
                cur->set_exception_on_blocking_promises(std::make_shared<canceled_exception>(
                        FC_LOG_MESSAGE(error, "cancellation reason: thread quitting")));
        ...
        for (task_base *unstarted_task : my->task_pqueue) {
            unstarted_task->set_exception(std::make_shared<canceled_exception>(
                    FC_LOG_MESSAGE(error, "cancellation reason: thread quitting")));
        }
        my->task_pqueue.clear();

        for (task_base *scheduled_task : my->task_sch_queue) {
            scheduled_task->set_exception(std::make_shared<canceled_exception>(
                    FC_LOG_MESSAGE(error, "cancellation reason: thread quitting")));
        }
    ...
    int thread::wait_any_until(std::vector<promise_base::ptr> &&p, const time_point &timeout) {
        ...
        if (timeout != time_point::maximum()) {
            ...
            std::push_heap(my->sleep_pqueue.begin(), my->sleep_pqueue.end(), sleep_priority_less());
        }
    ...
    void thread::notify_task_has_been_canceled() {
        async([=]() {
            my->notify_task_has_been_canceled();
