...
namespace fc 
{
    ...
    uint128lh_t::uint128lh_t(const std::string &sz)
    :lo(0), hi(0)
    {
    ...
    uint128lh_t::operator bigint()const
    {
       auto tmp  = uint128lh_t( bswap_64( hi ), bswap_64( lo ) );
       bigint bi( (char*)&tmp, sizeof(tmp) );
       return bi;
    }
    uint128lh_t::uint128lh_t( const fc::bigint& bi )
    {
       *this = uint128lh_t( std::string(bi) ); // TODO: optimize this...
    }

    uint128lh_t::operator std::string ()const
    {
      if(*this == 0) { return "0"; }
      ...
      uint128lh_t remainder;
      divide(ii, uint128lh_t(10), ii, remainder);
    ...
    uint128lh_t& uint128lh_t::operator<<=(const uint128lh_t& rhs)
    {
        if(rhs >= 128) 
        {
          hi = 0;
    ...
    uint128lh_t& uint128lh_t::operator/=(const uint128lh_t& b)
    {
   ...
   void uint128lh_t::full_product(const uint128lh_t& a, const uint128lh_t& b, uint128lh_t& result_hi, uint128lh_t& result_lo)
   {
       ...
       result_hi = uint128lh_t(y[3], y[2]);
       result_lo = uint128lh_t(y[1], y[0]);
   ...
   uint8_t uint128lh_t::popcount()const
   {
      return _popcount_64( lo ) + _popcount_64( hi );
   }

   void to_variant(const uint128lh_t& var, variant& vo)  { vo = std::string(var);         }
   void from_variant(const variant& var, uint128lh_t& vo){ vo = uint128lh_t(var.as_string()); }
