...
//      wdump((result));
...

      {
         ...
         BOOST_CHECK( !fc::ecc::verify_sum( {C2}, {C3} ) );
         BOOST_CHECK( fc::ecc::verify_sum( {C1}, {C1}) );
         BOOST_CHECK( fc::ecc::verify_sum( {C2}, {C2} ) );
         BOOST_CHECK( !fc::ecc::verify_sum( {C3}, {C2} ) );
         BOOST_CHECK( !fc::ecc::verify_sum( {C1}, {C2} ) );
         BOOST_CHECK( !fc::ecc::verify_sum( {C1}, {C2} ) );
         BOOST_CHECK( !fc::ecc::verify_sum( {C2}, {C1} ) );
      }
...

      {
         ...
         BOOST_CHECK( !fc::ecc::verify_sum( {InC}, {In0} ) );
