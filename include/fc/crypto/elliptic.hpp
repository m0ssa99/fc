...
namespace fc {
    ...
    namespace ecc {
        ...
        public:
            extended_public_key(const public_key &k, const sha256 &c, int child = 0, int parent_fp = 0,
                                uint8_t depth = 0);
        ...
        public:
            extended_private_key(const private_key &k, const sha256 &c, int child = 0, int parent_fp = 0,
                                 uint8_t depth = 0);
            ...
            compact_signature unblind_signature(const extended_public_key &bob, const blind_signature &sig,
                                                const fc::sha256 &hash, int i) const;

        private:
            extended_private_key private_derive_rest(const fc::sha512 &hash, int num) const;
        ...
        range_proof_type range_proof_sign(uint64_t min_value, const commitment_type &commit,
