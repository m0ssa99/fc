...
namespace fc {
    namespace detail {
        ...
        public:
            ...
            gntp_icon_impl(const char *buffer, size_t length) : _icon_bytes(buffer, length),
        ...
        gntp_notifier_impl::gntp_notifier_impl(const std::string &host_to_notify /* = "127.0.0.1" */,
                                               uint16_t port /* = 23053 */,
                                               const optional<std::string> &password /* = optional<std::string>() */)
                : hostname(host_to_notify), port(port), password(password), connection_failed(false),
                is_registered(false) {
        }
    ...
    gntp_icon::gntp_icon(const char *buffer, size_t length) : my(new detail::gntp_icon_impl(buffer, length)) {
    }
    ...
    gntp_notifier::gntp_notifier(const std::string &host_to_notify /* = "127.0.0.1" */, uint16_t port /* = 23053 */,
                                 const optional<std::string> &password /* = optional<std::string>() */) : my(
            new detail::gntp_notifier_impl(host_to_notify, port, password)) {
    }
