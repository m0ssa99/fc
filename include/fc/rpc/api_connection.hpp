...
namespace fc {
   ...
   class api_connection : public std::enable_shared_from_this<fc::api_connection>
   {
      public:
         ...
         template<typename T>
         api<T> get_remote_api( api_id_type api_id = 0, std::string api_name = "" ) {
            api<T> result;
            if( api_name.size() )
         ...
         virtual variant send_call( std::string api_name, std::string method_name, variants args = variants() ) = 0;
      ...
      private:
         ...
         struct api_visitor {
            ...
            api_visitor( uint32_t api_id, std::string& api_name, std::shared_ptr<fc::api_connection> con )
   ...
   class local_api_connection : public api_connection
   {
      public:
         ...
         virtual variant send_call( std::string api_name, std::string method_name, variants args = variants() ) override {
